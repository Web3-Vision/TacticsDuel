...
export default async function NewsPage() {
  ...
  if (!user) {
    return (
      <div className="p-4">
        <p className="font-mono text-sm text-text-mid">Log in to access division news.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 pb-20 space-y-3">
      <header className="flex items-center justify-between gap-2">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-text-dim">Narrative Stream</p>
          <h1 className="font-mono text-lg text-text mt-1">News Wire</h1>
        </div>
      ...
      <section className="glass-panel rounded-xl p-3">
        <NarrativeFeed items={narrative.news} maxItems={12} emptyLabel="No news updates right now." />
      </section>
    </div>
  );
}
