...
export default function MatchdayClient({ initialMatchId }: MatchdayClientProps) {
  ...
  const autoConnectedMatchIdRef = useRef("");
  const matchIdFromQuery = initialMatchId;
  ...
  const hydrateFromStorage = useCallback(async () => {
    ...
    try {
      ...
      setRoomCodeInput(restoredSession.roomCode);
      if (restoredSession.matchId) {
        setMatchIdInput(restoredSession.matchId);
      }
  ...
  const refreshSession = useCallback(async () => {
    ...
    try {
      ...
      setRoomCodeInput(refreshed.roomCode);
      if (refreshed.matchId) {
        setMatchIdInput(refreshed.matchId);
      }
    } catch (refreshError) {
  ...
  useEffect(() => {
    ...
    setMatchIdInput(matchIdFromQuery);
  }, [matchIdFromQuery]);

  useEffect(() => {
    if (!matchIdFromQuery || autoConnectedMatchIdRef.current === matchIdFromQuery) {
      return;
    }

    if (session?.matchId === matchIdFromQuery) {
      autoConnectedMatchIdRef.current = matchIdFromQuery;
      return;
    }

    autoConnectedMatchIdRef.current = matchIdFromQuery;
    setBusyAction("connect_match");
    setError("");

    void sessionPost({ action: "connect_match", matchId: matchIdFromQuery })
      .then((connectedSession) => {
        setSession(connectedSession);
        setSessionIdInput(connectedSession.id);
        setRoomCodeInput(connectedSession.roomCode);
        setMatchIdInput(matchIdFromQuery);
        saveSessionId(connectedSession.id);
        setInfo(`Connected to fixture ${connectedSession.roomCode} for match ${matchIdFromQuery}.`);
      })
      .catch((connectError) => {
        autoConnectedMatchIdRef.current = "";
        setError((connectError as Error).message);
      })
      .finally(() => {
        setBusyAction(null);
      });
  }, [matchIdFromQuery, session?.matchId]);
